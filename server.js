...
app.get('/api/hotels', async (req, res) => {
  try {
    ...
    if (!RAPIDAPI_KEY) {
      return res.status(500).json({ error: "RAPIDAPI_KEY missing on Render Environment" });
    }

    // Step A: Region ID Fetching (Standardized parameters)
    ...
    if (regions.length === 0) {
      return res.status(404).json({ error: `Region for '${city}' not found.` });
    }
    ...
    if (!gaiaId) {
      return res.status(404).json({ error: "Unable to extract region gaiaId." });
    }
    ...
    res.status(500).json({ error: "RapidAPI Request Failed", details: errData });
  }
});

// 2. Explicit Root Route '/' to fix "Cannot GET /"
app.get('/', (req, res) => {
  res.send(`
    <!DOCTYPE html>
    <html lang="en">
    ...
    <body>
      <div class="container">
        <h2 style="color:#333; margin-top:0;">Sheet Holidays Search Engine</h2>
        <div class="search-box">
          <input type="text" id="cityInput" placeholder="Enter City (e.g. Mumbai, Goa, Delhi)">
      ...
      <script>
        async function searchHotels() {
          ...
          resultsDiv.innerHTML = "<p style='color:#666;'>Searching best deals for <b>" + city + "</b>...</p>";
          ...
          try {
            ...
            if (!properties || properties.length === 0) {
              resultsDiv.innerHTML = "<p style='color:#e65100;'>No hotels found for <b>" + city + "</b>.</p>";
              return;
            }
