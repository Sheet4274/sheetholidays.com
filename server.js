...
app.get('/api/destinations', async (req, res) => {
  try {
    ...
    if (!query) { 
      return res.status(400).json({ status: false, error: 'Please enter a destination' }); 
    } 
    const response = await axios.get(
      `https://${HOST}/api/v1/hotels/searchDestination`, 
      { params: { query: query, locale: 'en-us' }, headers }
    ); 
    res.json(response.data); 
  } catch (error) {
    console.error('Destination Error:', error.response?.data || error.message); 
...
app.get('/api/hotels', async (req, res) => {
  try {
    const { dest_id, search_type, checkin, checkout, adults = 2, rooms = 1 } = req.query; 
    if (!dest_id) { 
      return res.status(400).json({ status: false, error: 'dest_id is required' }); 
    } 
    if (!checkin || !checkout) { 
      return res.status(400).json({ status: false, error: 'checkin and checkout are required' }); 
    } 
    const response = await axios.get(
...
app.get('/api/flights', async (req, res) => {
  try {
    const { fromId, toId, departDate, returnDate, adults = 1, currency = 'INR' } = req.query; 
    if (!fromId || !toId || !departDate) { 
      return res.status(400).json({ status: false, error: 'fromId, toId and departDate are required' }); 
    } 
    const params = { 
      fromId: fromId.includes('.AIRPORT') ? fromId : `${fromId}.AIRPORT`, 
      toId: toId.includes('.AIRPORT') ? toId : `${toId}.AIRPORT`, 
